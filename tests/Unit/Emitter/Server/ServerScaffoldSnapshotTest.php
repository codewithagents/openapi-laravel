 ...
 * query, path, header, and inline request-body Data classes, and the routes
 * file) for the real petstore-3.0 spec, so the generated API shape is
 * diff-visible. The collector is wired with the model generator exactly like
 * the planner does, so the query classes (issue #63), path classes (issue
 * #113), header classes (issue #121), and inline-body classes (issue #76) are
...
it('matches the committed server-scaffold snapshot for petstore-3.0', function () {
    ...
    $combined = implode("\n", array_map(fn ($f) => $f->code, $controllers))
        ...
        ."\n".implode("\n", array_map(fn ($f) => $f->code, $generator->pathFiles()))
        ."\n".implode("\n", array_map(fn ($f) => $f->code, $generator->headerFiles()))
