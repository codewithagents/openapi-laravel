...
**Headline:** 19 construct(s) FULLY ENFORCED, 5 tracked KNOWN-GAP, out of 24 total.
...
- **MixedTypeEnum** (#bool-enum), MixedTypeEnumData: expected `accept`, actual `reject`. A boolean member of a mixed-type enum is dropped from Rule::in (enumValues() keeps only string|int|float), so a spec-valid boolean value is false-rejected. Narrow edge: only a heterogeneous enum that mixes a boolean with other scalar types.
...
| - | --------- | ----- | -------- | ----- | -------- | ------ | ------- |
| 1 | MixedTypeEnum | MixedTypeEnumData | (valid payload) | a boolean enum member must be accepted (true) | accept | reject | `{"value":true}` |
| 2 | NullableMixedOneOf | NullableMixedOneOfData | (valid payload) | a present null satisfies the nullable oneOf member | accept | reject | `{"value":null}` |
| 3 | OneOfDiscriminated (variant GadgetAlpha) | GadgetAlphaData | discriminator.const | a variant whose discriminator const does not match its own value must be rejected | reject | accept | `{"kind":"beta","alphaField":"x"}` |
| 4 | Exerciser (maps/arrays/aliases) | ExerciserData | oneOf.no-match | an object matching no union variant must be rejected (objectUnion) | reject | accept | `{"objectUnion":{"nope":1}}` |
| 5 | AdditionalPropsFalse | AdditionalPropsFalseData | additionalProperties:false | an undeclared extra property must be rejected | reject | accept | `{"known":"x","extra":"y"}` |
