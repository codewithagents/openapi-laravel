...
- **Server scaffold** (generated by default, opt out with `--no-controllers` / `--no-routes`) → an
  ...
  operation is a PHP fatal and path-level drift is structurally impossible. Method and route names
  follow the Laravel conventions (`index`/`show`/`store`/`update`/`destroy` for clean RESTful
  operations, deterministic fallback to the operationId-derived name for everything else)
...
- **Tag-grouped data layout** → Data classes and enums solely owned by one tag live in per-tag
  subdirectories with namespaces following the directories (`data/Pet/PetData.php`,
  `App\Data\Pet`), mirroring the controller grouping; schemas several tags share stay at the flat
  root, cross-group references are imported, and the attribution is deterministic
