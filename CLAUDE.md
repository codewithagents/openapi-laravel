...
classes (#76), but an inline body that is not an object shape (array, scalar, union, enum,
free-form map) keeps the warned Request fallback; tuple `prefixItems` validates per position (#82,
incl. a length cap for the closed `items: false` form) but still types as `array<int, mixed>`;
int64 bounds and non-JSON responses degrade gracefully.
