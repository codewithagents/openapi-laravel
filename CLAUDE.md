...
Opt-in tag-grouped data layout (#93, `--group-by-tag` / `output.group_by_tag`, default off): Data
classes and enums solely owned by one tag group (first-tag rule, transitive closure walk) move
into per-tag subdirectories with namespaces following the directories, mirroring the controller
grouping; multi-tag, unreferenced, and reserved-`Support`-tag schemas stay at the flat root;
per-operation query/body classes follow their operation's tag; cross-group references are
imported; the flat default stays byte-identical.
