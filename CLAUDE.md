...
dropped (a bad value is a 422, not a 200); both run the exact body-class rules pipeline through one
location-parameterized core (the seam #121 header support will extend). The path class is additive:
the positional scalar path args still fill the signature, and the abstract method carries a docblock
pointer to `::fromRoute($request)` rather than injecting it. An inline JSON object request body
