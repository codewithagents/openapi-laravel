...
| PUT full-replace + readOnly-preserve | `PUT /pet` (`updatePet`) over `PetWritableData` | a PUT that OMITS optional fields full-REPLACES (omitted `attributes`/`weight_kg`/`microchip_id` null out, no merge) while the readOnly `created_at` is PRESERVED unchanged | generated write variant (no readOnly field) + consumer rebuilds the read model field-by-field and re-sources `created_at` from the existing pet | proven (genuine replace, not merge) |
...
| nested binary multipart (#75 residual) | `multipart/form-data` with a `format: binary` field NESTED inside an object property (`wrapper.payload`) | the nested binary stays a PLAIN STRING (typed `string`, `'string'` rule, NO `UploadedFile`/`file`/`mimetypes`): a plain-text value is accepted and echoed verbatim, where a root-level file part would 422 | generated nested `*Data` types the nested binary as `string` (only ROOT-level binary becomes `UploadedFile`) | proven (documented residual: nested binary is not file-handled) |
...
| delimited array query param (#132) | `GET /lab/delimited-query` with three non-exploded delimited array params: `csv` (form, `explode: false`, integer items `min:1 max:100`), `ssv` (`spaceDelimited`), `psv` (`pipeDelimited`) | each joined value is SPLIT in the generated `fromQuery()` factory on its style's delimiter BEFORE the array rules run (comma/space/pipe), so `?csv=1,2,3` round-trips as a 3-element array and an out-of-range csv item 422s keyed `csv.N`; a missing required delimited param 422s. The split values stay STRINGS (`explode()` yields strings, laravel-data does not cast array elements; the `integer` rule validates integer-ness but does not coerce) | generated `LabDelimitedQueryQueryData::fromQuery()` split + `csv.*` per-item rules. The op is a body-less GET (the real-world Stripe-style filter case): a delimited-array query class is now ADDITIVE even on a GET (the abstract takes no injected param and points at `fromQuery($request)`), so the split is the runtime-live path instead of spatie validating the unsplit string first | proven |
| deepObject object query param (#131) | `GET /lab/deep-filter` with a `style: deepObject, explode: true` OBJECT param `filter` whose schema has `gte` integer `min:0`, `lte` integer `max:100`, `color` string `enum` | the real-world Stripe-style `?filter[gte]=10&filter[lte]=20&filter[color]=red` hydrates the nested object and echoes `{gte,lte,color}` (200); an out-of-range nested value (`filter[gte]=-1`, `filter[lte]=999`) or an out-of-enum `filter[color]` 422s keyed on the DOTTED nested path (`filter.gte`, `filter.lte`, `filter.color`); a missing required `filter` 422s | generated `LabDeepFilterQueryData` with a NESTED `LabDeepFilterQueryFilterData` property carrying dotted nested rules. Unlike #132, no split and no `fromQuery()` pointer: PHP parses the bracketed query keys natively into a nested array, so the param stays container-INJECTABLE on a body-less GET (the abstract INJECTS `LabDeepFilterQueryData` directly) | proven |
...
Note: `GET /lab/styles` used to be the fifth residual pin (a `deepObject` `filter`
object plus a `pipeDelimited` `ids` array, both skipped with warnings). Both have
since been implemented (`ids` by #132, `filter` by #131), the skip warnings are
GONE, and the op is no longer a residual: it is now a second proof of the
#131/#132 query-style flips alongside the dedicated `/lab/delimited-query` (#132)
and `/lab/deep-filter` (#131) ops. Its test asserts the supported behavior (both
styles generate and validate) rather than a skip.
