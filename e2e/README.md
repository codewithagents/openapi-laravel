...
|---|---|---|---|
| `GET /lab/styles`: a `deepObject` exploded object query param (`filter`) + a `pipeDelimited` array query param (`ids`), alongside a normal `page` int param | The `deepObject` `filter` is STILL skipped with a warning and never appears in the generated query class. The `pipeDelimited` `ids` is NO LONGER skipped (#132) but this op is a body-less GET, so the generator AUTO-INJECTS the query class and spatie validates it against the RAW request, bypassing `fromQuery()`: the injected `ids` is judged as the unsplit joined STRING and fails the `array` rule, so any `ids` value 422s on this GET. The supported `page` param is still validated. | The op 200s with a garbage `filter` value and `ids` OMITTED (filter does not gate the request); ANY `ids` value (valid-looking or not) 422s with `ids` in the error bag (the split is shadowed on the injected GET path); `page` is still validated (min:1/max:50). The runtime-live split proof is on POST `/lab/delimited-query`. | `query parameter "filter" was skipped: style "deepObject" is not supported yet.` (the `ids` pipeDelimited skip warning is GONE: #132 now generates and splits it) |
