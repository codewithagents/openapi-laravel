...
|---|---|---|---|
| `GET /lab/styles`: a `deepObject` exploded object query param (`filter`) + a `pipeDelimited` array query param (`ids`), alongside a normal `page` int param | The `deepObject` `filter` is STILL skipped with a warning and never appears in the generated query class. The `pipeDelimited` `ids` is NO LONGER a residual (#132): it is generated AND splits correctly on this body-less GET, because a delimited-array query class is now ADDITIVE even on a GET (the abstract takes no injected param and points at `fromQuery($request)`), so the pipe split runs BEFORE the integer-array rules instead of spatie validating the unsplit string first. The supported `page` param is still validated. | The op 200s with a garbage `filter` value (it does not gate the request); a valid `ids=99\|7` round-trips as the split array `["99","7"]`; a non-integer `ids` item 422s keyed `ids.N`; `page` is still validated (min:1/max:50). `filter` stays a documented residual; the `ids` row is now a green proof, not a residual. | `query parameter "filter" was skipped: style "deepObject" is not supported yet.` (the `ids` pipeDelimited skip warning is GONE: #132 now generates and splits it) |
