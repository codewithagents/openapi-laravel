...
`openapi-laravel` reads standard OpenAPI with no custom extensions, and emits `laravel-data` classes with explicit `rules()` and native enums.
...
- **You need full server scaffolding today:** that is on the roadmap here as v2, not shipped in v1. See the [roadmap](/roadmap).
