...
The `oneOf` / `anyOf` cell is honest: the generator emits native PHP union type hints where the members resolve, but an **object union** (`CatData|DogData`) does not auto-hydrate at runtime without a `discriminator`. See the [limitations page](/guides/limitations#oneof-and-anyof-emit-union-types).
