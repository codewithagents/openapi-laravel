...
- the abstract controllers (`Abstract*Controller.php`), when `--controllers` is on,
- the generated routes file, when `--routes` is on.
...
`openapi:check` honours the same config and the same flags as `openapi:generate` (`--spec`, `--output`, `--namespace`, `--controllers`, `--routes`), so it always checks exactly what generate would produce.

<Tabs>
<TabItem label="Laravel (artisan)">

```bash
# Models only (uses config/openapi-laravel.php)
php artisan openapi:check

# Override the spec and output, and include the server scaffold
php artisan openapi:check \
  --spec=openapi.yaml \
  --output=app/Data \
  --controllers \
...
# With the server scaffold
vendor/bin/openapi-laravel check \
  --spec=openapi.yaml \
  --output=app/Data \
  --controllers --controller-output=app/Http/Controllers/Api \
  --routes --routes-output=routes/api.generated.php
```
...
| `2`  | Configuration or spec error (no spec configured, an unparseable spec, a missing output path, an illegal namespace). |
  ...
  drift-check:
    ...
    steps:
      ...
      - name: Verify generated code is in sync with the spec
        run: php artisan openapi:check --controllers --routes
```
