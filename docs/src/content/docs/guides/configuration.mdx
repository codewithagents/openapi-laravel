...
return [
    ...
    'output' => [
        ...
        /*
         * Opt-in: emit Data classes into per-tag subdirectories with
         * namespaces following the directories (data/Pet/PetData.php under
         * App\Data\Pet), mirroring the controller grouping.
         */
        'group_by_tag' => false,
    ],
...
Opt-in tag-grouped data layout. When `true`, generated Data classes and enums that are solely owned
by one tag are emitted into a per-tag subdirectory, with the namespace following the directory:
`data/Pet/PetData.php` declares `App\Data\Pet\PetData`. The grouping mirrors the per-tag controller
grouping, so a 176-path spec no longer piles 128 classes into one flat directory. Defaults to
`false`: the flat layout, byte-identical to previous releases. Overridden per run by
`--group-by-tag` / `--no-group-by-tag` on `openapi:generate`, `openapi:check`, `openapi:scaffold`,
and the standalone binary (passing both together exits `2`).

The schema-to-tag attribution rule is deterministic; see
[the grouped data layout](/guides/generated-output#tag-grouped-data-layout-opt-in) for the full
rule table. Cross-group references are imported, so the generated code compiles in either layout,
and `openapi:check` verifies whichever layout the same flags and config select.

<Aside type="caution">
  Toggling the layout moves files. When switching an existing project, delete the previously
  generated Data files first: `output.prune` only clears the top level of the output directory,
  so files from the other layout would linger as stale duplicates.
</Aside>
...
| `laravel_conventions` | `false`                            | Opt in to conventional `index`/`show`/`store`/`update`/`destroy` method and route names for clean RESTful operations. Overridden by `--laravel-conventions` / `--no-laravel-conventions`. See the [server scaffold guide](/guides/server-scaffold#laravel-convention-method-names-opt-in) for the mapping table and the fallback rule. |

When enabled, the generator writes one `Abstract{Tag}Controller` file per OpenAPI tag into `path`. Each file is overwritten on every run. Your concrete controllers (which extend the abstract classes) are never touched. See [custom base class](/guides/server-scaffold#custom-base-class) for what `base_class` emits.
...
See [security middleware from the spec](/guides/server-scaffold#security-middleware-from-the-spec)
...
`--laravel-conventions` / `--no-laravel-conventions`, `--group-by-tag` / `--no-group-by-tag`,
