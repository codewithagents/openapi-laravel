...
return [
     ...
     * outside its declared property set. Off by default (lenient keeps
     * contract evolution safe); opt in here or with --enforce-closed-objects.
     */
...
When `true`, every schema declaring `additionalProperties: false` emits a rule that rejects any
input key outside its declared property set. Defaults to `false`: strict rejection has a
forward-compatibility hazard (a producer that adds a new field breaks a consumer that has not
regenerated yet), so lenient is the safe default. Opt in here, or per run with the
`--enforce-closed-objects` flag on `openapi:generate` / `openapi:check` (the standalone binary
accepts the flag too; the `openapi-laravel.json` file has no key for it). See
[the limitations guide](/guides/limitations#additionalproperties-false-enforceable-via-an-opt-in-flag)
