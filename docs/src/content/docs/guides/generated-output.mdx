...
```php
// e2e/backend/app/Data/Pet/PetData.php
...
```php
// e2e/backend/app/Data/Pet/PetWritableData.php
...
Data classes and enums are emitted in a per-tag layout that mirrors the controller grouping. A
...
<Aside type="caution">
  Upgrading from a release with the flat layout moves files. Delete the previously generated Data
  files before regenerating: `output.prune` only clears the top level of the output directory, so
  files from the old layout would linger as stale duplicates.
</Aside>

## Determinism

The generator emits stable ordering everywhere: properties, rules, enum cases, and file output. The same spec in produces byte-identical files out, so re-running in CI yields either no diff or a diff that is entirely a consequence of the spec changing. The [tag-grouped layout](#tag-grouped-data-layout) is equally deterministic.
...
- **Fidelity report** (`openapi-laravel.unsupported.json`): a single JSON file listing every OpenAPI construct in the spec the generator cannot faithfully represent. See [Unsupported constructs report](/guides/unsupported-report) for the shape, the recorded constructs, and the opt-out.
