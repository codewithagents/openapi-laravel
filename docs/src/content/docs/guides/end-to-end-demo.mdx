...
The `external_id` field exercises the `oneOf` / `anyOf` support. A scalar union is chosen deliberately: an **undiscriminated** object union does not auto-hydrate in `laravel-data` (a documented residual; a union with a `discriminator` is validated and hydrated, see the [limitations page](/guides/limitations#discriminated-object-unions-are-validated-and-hydrated)), whereas a scalar union round-trips cleanly and proves the union path end to end.
