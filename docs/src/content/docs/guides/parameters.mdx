...
<Aside type="note" title="A delimited-array query class is forced additive">
  A `<Operation>QueryData` that carries **any** non-exploded delimited-array parameter is **never
  container-injected**, even on a body-less `GET`. The abstract method takes no injected query param
  and instead carries a `::fromQuery($request)` docblock pointer, the exact mechanism path (#113) and
  header (#121) parameters use. Container injection would make spatie/laravel-data validate the
  **raw, unsplit** string before `fromQuery()` runs the split, 422-ing a valid body-less filter. A
  `deepObject` parameter needs no such treatment: PHP has already parsed the nested array, so a
  QueryData with only flat and deepObject parameters stays container-injectable on body-less
