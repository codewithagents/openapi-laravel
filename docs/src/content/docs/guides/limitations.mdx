...
listed here too; since issue #76 they synthesize typed per-operation body Data classes. Spec-declared
response status codes used to be listed here as well; since issue #64 they are honored, with the
residuals described under
...
An inline JSON **object** body no longer falls back: it synthesizes a per-operation
`<Operation>RequestData` class with the full `rules()` pipeline and a typed controller param
(issue #76, see the [server scaffold guide](/guides/server-scaffold#inline-request-bodies)). A
`multipart/form-data` **object** body no longer falls back either: it synthesizes the same class
with `UploadedFile` typing and `file` / `mimetypes:` rules for its binary parts (issue #75, see
[multipart bodies](/guides/server-scaffold#multipart-bodies-and-file-uploads)). The remaining
fallbacks:

- A body that is a `$ref` into `components.requestBodies` (not resolved yet; this is a separate
  residual from the inline case, reference the schema directly under `content` for a typed param).
- An inline JSON or multipart body that is **not an object shape**: an array, a scalar, a
  `oneOf`/`anyOf` union, an enum, or a free-form map (`additionalProperties` only). No single Data
  class can type these; for multipart this includes the whole-body binary form
  (`schema: {type: string, format: binary}`).
- A body with no `application/json` or `multipart/form-data` content (form-urlencoded,
  octet-stream, ...): there is no schema mapping to type, so read and validate it from the
  `Request` yourself.
...
- A response that is a `$ref` into `components.responses` (not resolved yet) warns, except when it
  is selected for a 204: the method is `void` either way, so nothing degrades.
- An external response schema `$ref` warns.
- A **local** response schema `$ref` to a non-Data component (a scalar or array alias, an enum, a
  map), or an inline response schema that is not an array-of-`$ref` or a clean Data-class union,
  falls back **without** a warning: only an object shape can type the return, and `JsonResponse` is
  the honest general-purpose return type for everything else.
