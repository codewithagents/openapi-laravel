...
Two scaffold-level gaps are known and tracked on the pre-1.0 milestone. Until the issues close,
plan around them in your concrete controllers.

### Query parameters are not typed or validated

Operation `parameters` with `in: query` (and `in: header` / `in: cookie`) are currently **dropped**:
they produce no typed controller arguments, no validation rules, and no warning. Only `in: path`
parameters become typed method arguments. A required query parameter, an enum filter, or a
pagination bound declared in the spec is not enforced by the generated scaffold; read and validate
query input yourself in the concrete controller until this lands. Tracked in
[issue #63](https://github.com/codewithagents/openapi-laravel/issues/63).
...
mapping and Laravel's `boolean` rule will reject the literals.
