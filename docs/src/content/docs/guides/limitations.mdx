...
### additionalProperties: false (enforceable via an opt-in flag)

By **default**, `additionalProperties: false` is **not enforced**: the named properties are generated
as usual and extra keys are simply not rejected. Standard Laravel has no native rule to reject unknown
keys, and, more importantly, enforcing a closed shape carries a forward-compatibility hazard. A
producer that adds a new field to its responses would break a consumer that has not regenerated its
Data classes yet, since the unknown field would be rejected. Lenient-by-default keeps that contract
evolution safe, so the default output stays exactly as it was.

When you control both ends of the contract and want the strict OpenAPI intent (reject any undeclared
key), opt in:

- artisan: `php artisan openapi:generate --enforce-closed-objects`
- standalone binary: `vendor/bin/openapi-laravel generate --enforce-closed-objects`
- config: set `'enforce_closed_objects' => true` in `config/openapi-laravel.php`

With the flag on, every schema that declares `additionalProperties: false` emits a single
`NoUnknownPropertiesRule` (shipped in the package) that rejects any input key outside the declared
property set. It runs once per object, even when the payload omits every declared property, so a
payload that is nothing but unknown keys is still rejected. Schemas that do not declare
`additionalProperties: false` are untouched, and the flag changes nothing for the default (off) build,
whose output is byte-identical to before.
...
When a named `oneOf`/`anyOf` component carries a `discriminator` and every member is a `$ref` to an
object schema, the generator emits a real discriminated union: an **abstract morphable base** plus
one **variant class** per member. spatie/laravel-data reads the discriminator value from the payload,
selects the concrete variant, and validates and hydrates **that** variant, so per-variant validation
and polymorphic hydration both work at runtime, with no custom cast.
...
emits a build warning explaining why. This happens when a member is not an object schema, when a
variant is shared by two discriminated bases (PHP single inheritance keeps the first base), or when a
base is left with no claimable variant. The remaining forms still presence-only: an **inline**
discriminated union written directly under a property (not a named component schema), and the
**allOf-inheritance** form where a base object declares properties plus a discriminator and variants
compose it with `allOf`. Both are tracked for a future phase.
...
`enum`), the server scaffold, scalar `oneOf`/`anyOf` union type hints, and discriminated object
...
(any object is accepted, no variant enforced) pending an inline/allOf-inheritance follow-up, and the
map caveats apply (raw-array `$ref` map values, uncaptured mixed-object overflow, and
`additionalProperties: false` left unenforced by default but enforceable via the opt-in
`--enforce-closed-objects` flag).
