...
(issue #76, see the [server scaffold guide](/guides/server-scaffold#inline-request-bodies)). The
remaining fallbacks:
...
- An inline JSON body that is **not an object shape**: an array, a scalar, a `oneOf`/`anyOf` union,
  an enum, or a free-form map (`additionalProperties` only). No single Data class can type these.
- A body with no `application/json` content (multipart, form-urlencoded, octet-stream, ...): there
  is no JSON schema to type, so read and validate it from the `Request` yourself.
