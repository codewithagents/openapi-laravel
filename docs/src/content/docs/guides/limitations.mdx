...
described under [Query parameters](#query-parameters) below. Spec-declared response status codes
used to be listed here too; since issue #64 they are honored, with the residuals described under
...
- A body that is a `$ref` into `components.requestBodies` (not resolved yet).
- An inline (non-`$ref`) JSON body schema (not generated yet).
