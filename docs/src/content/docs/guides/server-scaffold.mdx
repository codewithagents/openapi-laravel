...
```
openapi.yaml
  └── openapi-laravel --controllers --routes
...
## Enabling the scaffold

The scaffold is opt-in. Both the config file and the command-line flags give you control.

### Via config

Publish the config if you have not already, then enable the sections you want:
...
Pass `--controllers` and/or `--routes` on the command line without touching the config:

```bash
php artisan openapi:generate --controllers --routes
```

Additional flags for the artisan command:

| Flag                          | Description                                                    |
| ----------------------------- | -------------------------------------------------------------- |
| `--controllers`               | Generate abstract controllers                                  |
...
The standalone binary accepts the same flags plus two extra ones for controller output:
...
| `--controller-output`       | Directory the abstract controller files are written to         |
| `--routes-output`           | Path the routes file is written to                             |
