...
| spatie/laravel-data | v4, a **runtime** dependency of your app: `composer require spatie/laravel-data:^4.0` |
...
<Aside type="tip">
  ...
  command line. The output **namespace** is set only in the config file (`output.namespace`); the
  artisan command has no `--namespace` flag. The standalone binary does accept `--namespace`, because
  it has no config file to read from.
</Aside>
